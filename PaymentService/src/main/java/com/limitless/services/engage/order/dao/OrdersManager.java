...
public class OrdersManager {
	...
	public OrderPaymentModeUpdateResponseBean updatePaymentMode(int orderId, String paymentMode) {
		...
		try {
			...
			Orders order = (Orders) session.get("com.limitless.services.engage.order.dao.Orders", orderId);
			if (order != null) {
				responseBean.setPrevPaymentMode(order.getPaymentMode());
	...
	public OrderStatusResponseBean orderStatusUpdate(int orderId, String status) {
		...
		try {
			...
			session.update(order);
			Orders order2 = (Orders) session.get("com.limitless.services.engage.order.dao.Orders", orderId);
			responseBean.setOrderId(orderId);
				...
				if (orderList.size() > 0) {
					for (Orders order : orderList) {
						...
						CustomerAddressBook address = (CustomerAddressBook) session.get(
								"com.limitless.services.engage.dao.CustomerAddressBook", order.getDeliveryAddress());
						if (address != null) {
			...
			try {
				...
				if (!order.getOrderStatus().equals("PROCESS_FAILED")) {
					...
					if (detailsList.size() > 0) {
						for (OrderDetails detail : detailsList) {
							...
							Product product = (Product) session.get(
									"com.limitless.services.engage.sellers.product.dao.Product", detail.getProductId());
							...
							ProductCategory category = (ProductCategory) session.get(
									"com.limitless.services.engage.sellers.product.dao.ProductCategory", categoryId);
							if (category != null) {
								categoryName = category.getProductCategoryName();
							}
							ProductSubcategory subcategory = (ProductSubcategory) session.get(
							...
							mailContent += "<table><tr><td rowspan=3>" + "<img src=" + productImage
									+ " height=100 width=100>" + "</td><td><b>" + categoryName + "-" + subcategoryName
									+ "-" + productName + "</b>&nbsp;MRP Rs:" + productPrice + "</td>"
									+ "<td>Specs:<br>Color:" + productColor + "<br>Size:" + productSize
									+ "<td>Quantity:&nbsp;" + quantity + "</td>" + "<td>Discounted Price:&nbsp;"
									+ discountedPrice + "</td>" + "<td>Unit's Total Amount:&nbsp;" + totalPrice
									+ "</td></tr></table>";
						}
					}
					if (order.getPaymentMode() != null && order.getPaymentMode().equals("PAID")) {
						mailContent += "<br><h2>Total Amount Paid Rs. " + totalAmount + "</h2>" + "<br>" + "Notes: "
								+ txnNotes + "<br><br><b>Buyer Info:</b>" + "<br><i>Name: " + customer.getCustomerName()
								+ "</i>" + "<br><i>Mobile Number: " + customerMobile + "</i>" + "<br><i>Email: "
								+ customerEmail + "</i>" + "<br><br>" + "<b>Seller Info:</b>" + "<br><i>Shop Name: "
								+ sellerShopName + "</i>" + "<br><i>Phone Number: " + sellerMobile + "</i>"
						...
						mailContent += "<br><h2>Total Amount To Be Paid Rs. " + totalAmount + "<sup>*</sup></h2>"
								+ "<br>" + "<br>" + "<br><br><b>Buyer Info:</b>" + "<br><i>Name: "
								+ customer.getCustomerName() + "</i>" + "<br><i>Mobile Number: " + customerMobile
								+ "</i>" + "<br><i>Email: " + customerEmail + "</i>" + "<br><br>"
								+ "<b>Seller Info:</b>" + "<br><i>Shop Name: " + sellerShopName + "</i>"
								+ "<br><i>Phone Number: " + sellerMobile + "</i>" + "<br><i>Email: " + sellerEmail
								+ "</i>";
					}
					mailContent += "<br><br>" + "<b>Delivery Address</b><br>";
					if (order.getDeliveryAddress() == 0) {
						mailContent += "NA / IN-SHOP - DELIVERY AT THE SHOP";
					} else if (order.getDeliveryAddress() != 0) {
						mailContent += receiverName + "<br>" + addressLine1 + "<br>" + addressLine2 + "<br>" + city
								+ "<br>" + state + "<br>ZIP:" + zip + "<br>Mobile:" + receiverMobile + "<sup>**</sup>";
					...
					message.setContent(mailContent, "text/html");
						...
						|| order.getOrderStatus().equals("ORDER_CANCELED")) {
					message.setFrom(new InternetAddress("transactions@limitlesscircle.com"));
					...
					if (detailsList.size() > 0) {
						for (OrderDetails detail : detailsList) {
							...
							mailContent += "<table><tr><td rowspan=3>" + "<img src=" + productImage
									+ " height=100 width=100>" + "</td><td><b>" + productName + "</b>&nbsp;Price Rs:"
									+ productPrice + "</td>" + "<td>Quantity:&nbsp;" + quantity + "</td>"
	...
	public InventoryUpdateResponseBean updateInventory(int orderId) {
		...
		try {
			...
			if (detailsList.size() > 0) {
				for (OrderDetails details : detailsList) {
					ProductInventory inventory = (ProductInventory) session.get(
							"com.limitless.services.engage.sellers.product.dao.ProductInventory",
							details.getProductId());
					if (inventory != null) {
